    ...
    >
      ...
      {showDots && (
        <div className="mt-6 flex items-center justify-center gap-3">
            ...
            {items.map((it, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn("h-2 w-2 rounded-full transition", on ? "bg-marine-900" : "bg-marine-200 hover:bg-marine-400")}
          ...
          {activeItem.href ? (
            <Link href={activeItem.href} target="_blank" rel="noreferrer" className="text-marine-400 hover:text-marine-700 transition" aria-label="Abrir">
...
export function DefaultFanCard({ item }: { item: CardStackItem; active: boolean }) {
  return (
    <div
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-28" style={{ background: 'linear-gradient(to top, rgba(5,15,28,0.95) 0%, transparent 100%)' }} />

      {/* Text */}
      <div className="absolute inset-x-0 bottom-0 z-10 p-5">
        <div className="truncate text-[15px] font-bold text-white">{item.title}</div>
        {item.description && (
          <div className="mt-1 line-clamp-1 text-[12px] text-white/65">{item.description}</div>
        )}
      </div>
    </div>
  );
